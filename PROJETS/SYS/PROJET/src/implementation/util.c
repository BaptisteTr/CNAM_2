...
/**
 * print on standard output the passed parameter
 *
 * \param char * passed string
...
/**
 * print the current directory on standard output
 *
 * \return int 0 == SUCCESS
 */
int pwd(){
 ...
 *
 * \param char * directory target directory
 * \return int 0 == SUCCESS
 */
int changeDir(const char * directory){

  char * currentDir = malloc(BUFFER_SIZE);
  char * changeD = malloc(BUFFER_SIZE);
  char * res = malloc(BUFFER_SIZE);

  strcpy(changeD,directory);

  if(directory[0] == '/'){ // Corriger
    strcpy(res, changeD);
  } else {
    currentDir = getcwd(currentDir,BUFFER_SIZE);
    strcpy(res,currentDir);
    strcat(res,"/");
    strcat(res,changeD);
  }
